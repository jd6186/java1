package com.lec.java.collection08;

import java.util.HashSet;
import java.util.Iterator;

/* Set, HashSet<E>

	Collection<E>
	 |__ Set<E>
	      |__ HashSet<E>, TreeSet<E>
	
	 Set:
	 1. 자료의 중복 저장이 허용되지 않는다. (hashCode() 값의 중복여부!)    // List류와 결정적인 차이점이 이거다.
	 2. 저장 순서가 유지되지 않는다.(인덱스 없다.)  
	 (예) {1, 2, 3} = {1, 1, 2, 2, 3} : 중복 저장이 안되기 때문에 같은 Set
	 (예) {1, 2, 3} = {1, 3, 2}: 저장 순서가 중요하지 않기 때문에 같은 Set
	
	 HashSet: 매우 빠른 검색 속도를 제공 ★★★★★

	 	(※ HashXXX ← '검색속도 향상'을 쓰는 자료구조 입니다)
	 	
	 TreeXXX는 정렬에 유리하다.
*/

public class Collection08Main {

	public static void main(String[] args) {
		System.out.println("HashSet 클래스");
		
		// Integer 타입을 저장할 수 있는 HashSet 인스턴스 생성
		// TODO
		
		HashSet<Integer> set = new HashSet<Integer>();
		
		
		// 데이터 저장: add()
		// TODO
		set.add(100);
		set.add(200);
		set.add(300);
		set.add(400);
		set.add(100);
		set.add(500);
		set.add(300);
		set.add(1);
		set.add(3);
		
		
		// HashSet의 크기: size()
		// TODO
		System.out.println(set.size());
		
		
		// 위에 있는 것의 출력결과는 저장순서와는 관계가 없다.
//		itr.next(); // 이건 Runtime Exception 류의 NoSuchElementException이 뜬다.
		
		
		
		// Set은 중복 저장을 허용하지 않는다.
		
		// 데이터 검색:
		// Set은 인덱스가 없기 때문에 get() 메소드를 제공하지 않습니다.
		// 데이터 검색을 위해서는 Iterator를 사용해야 함
		
		
		System.out.println();
		set.iterator();
		Iterator<Integer> itr = set.iterator();
		while(itr.hasNext()) {
			System.out.println(itr.next());
		}
		
		
		
		
		// 데이터 삭제
		// remove(element): Set에 있는 element를 찾아서 삭제
		//   element 있다면 삭제 후 true를 리턴
		//   element 없다면 false 리턴
		// TODO
		System.out.println();
		System.out.println("삭제 후:");
		System.out.println();
		set.remove(100); // 100번째 인덱스를 삭제한게 아니라 100이라는 int를 삭제한거다.
		set.remove(990); // 없던 값을 지워도 에러는 아닌데 밑에처럼 출력해보면 false라고 뜬다.
		System.out.println("삭제결과 : " + set.remove(100));
		System.out.println("삭제결과 : " + set.remove(990));
		
		itr = set.iterator(); // 이터레이터 새로생성 왜? 이미 엘리먼트를 다 써서 이제 없거든 그래소 또 만들어줘야됑 위에 에러뜨잖아.NoSuchElementException
		while(itr.hasNext()) {
			System.out.println(itr.next());
		}
		
		
		// Set 자료형 데이터 변경을 할 수 있는 set() 메소드를 제공하지 않습니다.
		// 삭제(remove) 후 추가(add)하면 변경하는 효과
		// TODO
		
		set.remove(500);
		set.add(555);
		System.out.println("변경후");
		
		
		
		// enhanced-for 사용
		System.out.println();
		System.out.println("Enhanced for 사용 출력");
		// TODO
		
		for(Integer a : set) {
			System.out.println(a);
		}
		
		
		// forEach() 메소드 사용
		System.out.println();
		System.out.println("forEach() 사용 출력");
		// TODO
		
		System.out.println("\n프로그램 종료");
	} // end main()

} // end class Collection08Main












